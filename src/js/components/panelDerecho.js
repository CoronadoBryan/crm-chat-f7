export function panelDerecho(panelDatos, conv, perfil) {
    ...
    panelDatos.innerHTML = `
      <div class="block block-strong" style="margin-bottom:0; border-radius:16px; box-shadow:0 2px 8px rgba(0,0,0,0.06);">
        <div class="row" style="align-items:center;">
          ...
          <div class="col">
            <div style="font-size:22px;font-weight:700;letter-spacing:0.5px;">${perfil.nombre || ""} ${perfil.apellido || ""}</div>
            <div style="color:#666;font-size:15px;margin-top:2px;"><i class="icon f7-icons" style="font-size:15px;vertical-align:middle;">phone_fill</i> ${conv.telefono}</div>
          </div>
        </div>
      </div>
            ...
            <div class="item-content">
              ...
              <div class="item-inner">
                ...
                <div class="item-after">${
                  perfil.created_at?.date
                    ? new Date(perfil.created_at.date).toLocaleDateString()
                    : "-"
                }</div>
              </div>
            </div>
    ...
    panelDatos.innerHTML = `
      <div class="block block-strong" style="margin-bottom:0; border-radius:16px; box-shadow:0 2px 8px rgba(0,0,0,0.06);">
        <div class="row" style="align-items:center;">
          ...
          <div class="col">
            <div style="font-size:20px;font-weight:600;">Sin datos de perfil</div>
            <div style="color:#666;font-size:15px;margin-top:2px;"><i class="icon f7-icons" style="font-size:15px;vertical-align:middle;">phone_fill</i> ${conv.telefono}</div>
          </div>
        </div>
      </div>
      <div style="margin-top:20px; text-align:center;">
        <span class="chip color-red" style="font-weight:600;font-size:15px;"><i class="icon f7-icons" style="vertical-align:middle;">xmark_circle_fill</i> No hay perfil registrado</span>
      </div>
    `;
  }
}